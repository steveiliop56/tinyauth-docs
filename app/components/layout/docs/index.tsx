      ...
      <SidebarContent {...rest}>
        <SidebarHeader>
          <div className="flex flex-row gap-2.5 items-center">
            <img className="size-5" src="/icon/logo.png" alt="Tinyauth" />
            <Link
              href={nav.url ?? "/"}
  ...
  return (
    <TreeContextProvider tree={props.tree}>
      <NavProvider transparentMode={transparentMode}>
        {nav.enabled !== false &&
          (nav.component ?? (
            <Navbar className="h-(--fd-nav-height) on-root:[--fd-nav-height:56px] md:on-root:[--fd-nav-height:0px] md:hidden flex flex-row gap-2.5 items-center">
              <img className="size-5" src="/icon/logo.png" alt="Tinyauth" />
              <Link
                href={nav.url ?? "/"}
