...
Tinyauth's main purpose is to add a login screen to applications in homelabs or to facilitate resource sharing with family and friends. It is not designed for production environemnts which require features like RBAC and more granual permissions. For scenarios requiring advanced user management, Authentik is recommended.
