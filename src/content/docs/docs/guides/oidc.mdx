...
Due to the stateless nature of Tinyauth, the user `sub` is based on the client ID and the username. This means that if the username or client ID changes, the `sub` will also change. This can cause issues with some OIDC clients that rely on the `sub` claim to identify the user consistently.
...
If you are using the binary, you can specify the database directory using the `TINYAUTH_DATABASEPATH` environment variable or the `--databasepath` CLI flag.
...
We will use these values for the rest of this guide.

:::caution
The name of the client must be unique and alphanumeric; hyphens are allowed.
:::
