...
In order for Tinyauth to work with Zitadel we will need to use a custom OAuth provider, to do so, we will need to add the following environment variables to the Tinyauth docker container:
...
<Callout type="warning">
  OAuth doesn't mean security, with the current setup everybody with a Zitadel
