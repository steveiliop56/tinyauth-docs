...
Tinyauth has builtin support for Google OAuth and it is extremely easy to set up. Let's get started!
...
<Callout type="info">
  Google has changed the OAuth section and this guide uses the new OAuth
  experience. If you see a button saying try the new OAuth experience, please
  click it so that the guide looks the same as the Google Cloud Console.
</Callout>
...
<Callout type="info">
  You may see a warning saying that you haven't configured your OAuth Consent
  Screen, if you see this warning just refresh the page a couple of times and it
  should let you create your client.
</Callout>
...
<Callout type="warning">
  OAuth doesn't mean security, with the current setup everybody with a Github
...
<Callout type="tip">
  Since you have OAuth enabled, you can now remove the `USERS` or `USERS_FILE`
