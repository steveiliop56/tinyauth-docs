...
Tinyauth can be configured either with environment variables or CLI flags. The full list of configuration options is available below.

<Callout type="info">
  Every configuration option that has a `FILE_` equivalent (e.g. `USERS` and
  `USERS_FILE`), allows for the `FILE_` environment variable/CLI flag to be used
...
| `OAUTH_AUTO_REDIRECT`     | `--oauth-auto-redirect`     | Auto redirect to the specified OAuth provider (`github`, `google`, `generic`).                   | none              | no       |
...
The OAuth config includes a provider ID in the environment variable/flag name. For example `PROVIDERS_GOOGLE_CLIENT_ID`.
...
<Callout type="tip">
  If you use `google` or `github` as provider IDs, Tinyauth will automatically
  fill in all of the information needed (e.g. redirect URL, scopes etc.) for
  you.
</Callout>
## LDAP Config
...
<Callout type="tip">
  For Windows LDAP you will have to use the following search filter:
  `(&(sAMAccountName=%s))`.
</Callout>
